...
# Открываем кредитный счёт на созданного пользователя
...
# Создаём операцию покупки (платёж), привязанную к счёту и карте
make_purchase_operation_request = MakePurchaseOperationRequest(
    status=OperationStatus.OPERATION_STATUS_COMPLETED,
    amount=fake.amount(),
    card_id=open_credit_card_account_response.account.cards[1].id,
